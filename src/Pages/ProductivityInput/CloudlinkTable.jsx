...
function CloudlinkTable(props) {
    const [state, setState] = useState([]);
    useEffect(() => {
     
        const datacloud = props.data.userDropDownData.defaultDropDown['response'].productivity;
        const newobj = [];
        if (datacloud.cloudLevers[0].productivityPercentageByClientMaturityLevel) {
            datacloud.cloudLevers.forEach(ele => {
    ...
    return (
        <div className="devlinkclass">
            {props.data.userDropDownData.defaultDropDown['response'].productivity.cloudLevers[0].productivityPercentageByClientMaturityLevel ? <>
                <DataTable
                    rows={state}
                    headers={headers}>
