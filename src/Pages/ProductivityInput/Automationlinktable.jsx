...
function Automationlinktable(props) {
    const [state, setState] = useState([]);
    useEffect(() => {
       
        const dataAutomation = props.data.userDropDownData.defaultDropDown['response'].productivity;
        const newobj = [];
        dataAutomation.automationLevers.forEach(ele => {
    ...
    return (
        <div className="devlinkclass">
            {props.data.userDropDownData.defaultDropDown['response'].productivity.automationLevers[0].productivityPercentageByClientMaturityLevel ? <>
                <DataTable
                    rows={state}
                    headers={headers}>
...
export default Automationlinktable;
