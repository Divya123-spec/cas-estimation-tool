...
function DevSecOpslinktable(props) {
    const [state, setState] = useState([]);
    useEffect(() => {
    //   devscops liver making the new array with all devscops row 
        const datadevsecops = props.data.userDropDownData.defaultDropDown['response'].productivity;
        const newobj = [];
        datadevsecops.devSecOpsLevers.forEach(ele => {
    ...
    return (
        <div className="devlinkclass">
            {props.data.userDropDownData.defaultDropDown['response'].productivity.devSecOpsLevers[0].productivityPercentageByClientMaturityLevel ? <>
                <DataTable
                    rows={state}
                    headers={headers}>
...
export default DevSecOpslinktable;