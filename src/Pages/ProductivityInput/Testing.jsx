...
import Testinglink from './Testinglink';
import { CloseOutline16, CheckmarkOutline16, WarningFilled16, CheckmarkFilled16 } from "@carbon/icons-react";
import { testing_isScopeApplicablity } from "../../Components/CommonFuncs/formula/formulaProductivity_testing";
...
function Testing({ id, ...props }) {

  const dataProductivity = props.initialstateData.productivity.testing;
  ...
  const dropDownValueHandle1 = (id, val) => {
    let DropdownValue = '';
    props.userDropDownData.defaultDropDown['response'].productivity.testingLevers.map((ele) => {
      if (ele.code == id) {
  ...
  var newObj = [];
  initialstate.productivity.testing.testingLevers.forEach(ele => {
  ...
  let headerTitle = (<span className='AMSHeader'>Testing
  ...
  const uistatusValidation = (userInputData) => {
    let obj = {}
    let uiValidation = userInputData.productivity.testing.testingLevers.every((ele) => ele['uiStatus']['iconStatus']['ok']);
    uiValidation ? obj = {
      "iconStatus": {
        "warning": false,
        "ok": true,
    ...
    userInputData.productivity.testing['uiStatus'] = obj;
  }
  ...
  const dropdowncalculation = (initialstateData, cell) => {
    ...
    let newArray = initialstateData.productivity.testing.testingLevers.map((ele) => {
      if (id == ele.code) {
    ...
    initialstateData.productivity.testing.testingLevers = newArray;
  ...
  function handleDropDownChange(e, cell) {
    ...
    let initialStateProducitvity = [...initialstate.productivity.testing.testingLevers];
    ...
    initialStateData.productivity.testing.testingLevers = dataIntial;
    dropdowncalculation(initialStateData, cell.id);
    uistatusValidation(initialStateData);
    testing_isScopeApplicablity(props, initialStateData);
  ...
  const selectDropDownDynamic = (cell) => {
    var x = cell.id;
    let id = x.split(':')[0]
    let dataOptions = props.userDropDownData.defaultDropDown['response'].productivity.testingLevers.map((ele) => {
      if (ele.code == id) {
        return (ele.clientMaturityLevelOptions.map((item, index) =>
  ...
  const Validationhandler = (id) => {
    let warningClass = '';
    initialstate.productivity.testing.testingLevers.filter((ele) => {
      if (ele.code == id) {
  ...
  const expandedRowData = (cell) => {
    let dataOptions = initialstate.productivity.testing.testingLevers.map((ele) => {
      if (ele.code == cell) {
        return (
