...
import Automationlink from './Automationlink';
import { CloseOutline16, CheckmarkOutline16, CheckmarkFilled16, WarningFilled16 } from "@carbon/icons-react";
import { automation_isScopeApplicablity } from "../../Components/CommonFuncs/formula/formulaProductivity_automation";
...
function Automation({ id, ...props }) {

  const dataProductivity = props.initialstateData.productivity.automation;
  ...
  const dropDownValueHandle1 = (id, val) => {
    let DropdownValue = '';
    props.userDropDownData.defaultDropDown['response'].productivity.automationLevers.map((ele) => {
      if (ele.code == id) {
  ...
  var newObj = [];
  initialstate.productivity.automation.automationLevers.forEach(ele => {
  ...
  let headerTitle = (<span className='AMSHeader'>Automation
  ...
  const uistatusValidation = (userInputData) => {
    let obj = {}
    let uiValidation = userInputData.productivity.automation.automationLevers.every((ele) => ele['uiStatus']['iconStatus']['ok']);
    uiValidation ? obj = {
      "iconStatus": {
        "warning": false,
        "ok": true,
    ...
    userInputData.productivity.automation['uiStatus'] = obj;
  }
  ...
  const dropdowncalculation = (initialstateData, cell) => {
    ...
    let newArray = initialstateData.productivity.automation.automationLevers.map((ele) => {
      if (id == ele.code) {
    ...
    initialstateData.productivity.automation.automationLevers = newArray;
  }
  ...
  function handleDropDownChange(e, cell) {
    ...
    let initialStateProducitvity = [...initialstate.productivity.automation.automationLevers];
    ...
    initialStateData.productivity.automation.automationLevers = dataIntial;

    dropdowncalculation(initialStateData, cell.id);
    uistatusValidation(initialStateData);
    automation_isScopeApplicablity(props, initialStateData);
  ...
  const selectDropDownDynamic = (cell) => {
    var x = cell.id;
    let id = x.split(':')[0]
    let dataOptions = props.userDropDownData.defaultDropDown['response'].productivity.automationLevers.map((ele) => {
      if (ele.code == id) {
        return (ele.clientMaturityLevelOptions.map((item, index) =>
  ...
  const Validationhandler = (id) => {
    let warningClass = '';
    initialstate.productivity.automation.automationLevers.filter((ele) => {
      if (ele.code == id) {
  ...
  const expandedRowData = (cell) => {
    let dataOptions = initialstate.productivity.automation.automationLevers.map((ele) => {
      if (ele.code == cell) {
        return (
