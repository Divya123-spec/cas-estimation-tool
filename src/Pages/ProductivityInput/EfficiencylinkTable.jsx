...
function EfficiencylinkTable(props) {
    const [state, setState] = useState([]);
    useEffect(() => {
      
        const dataefficiency = props.data.userDropDownData.defaultDropDown['response'].productivity;
        const newobj = [];
        dataefficiency.efficiencyLevers.forEach(ele => {
    ...
    return (
        <div className="devlinkclass">
            {props.data.userDropDownData.defaultDropDown['response'].productivity.efficiencyLevers[0].productivityPercentageByClientMaturityLevel ? <>
                <DataTable
                    rows={state}
                    headers={headers}>
...
export default EfficiencylinkTable;
