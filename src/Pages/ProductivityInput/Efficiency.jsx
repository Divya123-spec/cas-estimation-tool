...
import Efficiencylink from './Efficiencylink';
import { CloseOutline16, CheckmarkOutline16, CheckmarkFilled16, WarningFilled16 } from "@carbon/icons-react";
import { efficiency_isScopeApplicablity } from "../../Components/CommonFuncs/formula/formulaProductivity_efficency";
...
function Efficiency({ id, ...props }) {

  const dataProductivity = props.initialstateData.productivity.efficiency;
  ...
  const dropDownValueHandle1 = (id, val) => {
    let DropdownValue = '';
    props.userDropDownData.defaultDropDown['response'].productivity.efficiencyLevers.map((ele) => {
      if (ele.code == id) {
  ...
  var newObj = [];
  initialstate.productivity.efficiency.efficiencyLevers.forEach(ele => {
  ...
  let headerTitle = (<span className='AMSHeader'>Efficiency
  ...
  const uistatusValidation = (userInputData) => {
    let obj = {}
    let uiValidation = userInputData.productivity.efficiency.efficiencyLevers.every((ele) => ele['uiStatus']['iconStatus']['ok']);
    uiValidation ? obj = {
      "iconStatus": {
        "warning": false,
        "ok": true,
    ...
    userInputData.productivity.efficiency['uiStatus'] = obj;
  }
  ...
  const dropdowncalculation = (initialstateData, cell) => {
    ...
    let newArray = initialstateData.productivity.efficiency.efficiencyLevers.map((ele) => {
      if (id == ele.code) {
    ...
    initialstateData.productivity.efficiency.efficiencyLevers = newArray;
  ...
  function handleDropDownChange(e, cell) {
    ...
    let initialStateProducitvity = [...initialstate.productivity.efficiency.efficiencyLevers];
    ...
    initialStateData.productivity.efficiency.efficiencyLevers = dataIntial;
    dropdowncalculation(initialStateData, cell.id);
    uistatusValidation(initialStateData);
    efficiency_isScopeApplicablity(props, initialStateData);
  ...
  const selectDropDownDynamic = (cell) => {
    var x = cell.id;
    let id = x.split(':')[0]
    let dataOptions = props.userDropDownData.defaultDropDown['response'].productivity.efficiencyLevers.map((ele) => {
      if (ele.code == id) {
        return (ele.clientMaturityLevelOptions.map((item, index) =>
  ...
  const expandedRowData = (cell) => {
    let dataOptions = initialstate.productivity.efficiency.efficiencyLevers.map((ele) => {
      if (ele.code == cell) {
        return (
  ...
  const Validationhandler = (id) => {
    let warningClass = '';
    initialstate.productivity.efficiency.efficiencyLevers.filter((ele) => {
      if (ele.code == id) {
            ...
            {({ rows, headers, getHeaderProps, getTableProps, getRowProps, getSelectionProps, }) => (
              <TableContainer title={headerTitle}>
                <div className="BriefEstimateModal">
                  ...
                  <Efficiencylink {...props}></Efficiencylink>
                </div>
                <Table {...getTableProps()}>
                  <TableHead>
                    <TableRow>
